...
class RealSensePCSubscriber(Node):
    ...
    def pointcloud_callback(self, msg: PointCloud2):
        ...
        points = [p for p in points if self.max_distance > np.linalg.norm(p)]
        ...
        avg = np.mean(filtered_points, axis=0)
        cube_x = float(avg[0])
        cube_y = float(avg[1])
        cube_z = float(avg[2])

        # filtered_points = filtered_points[np.linalg.norm(filtered_points - avg, axis=1) < 350]
